...
| “What work would you love to do?” | Seeker / Both | versioned `seeker_needs` JSONB | Structured multi-select; section-specific Other is conditional and optional | Continue / deferred Step 3 | Separate seeker taxonomy, deterministic need summary, demand analytics and CSV |
| “Who are you looking to hire?” | Recruiter / Both | versioned `recruiter_needs` JSONB | Structured multi-select; section-specific Other is conditional and optional | Continue / deferred Step 3 | Separate recruiter taxonomy, deterministic need summary, demand analytics and CSV |
| “How you like to work” | Seeker / Both | `work_formats` | Optional multi-select | Continue / deferred Step 5 | Seeker matching context and CSV |
| “What best describes you?” | Recruiter / Both | `organisation_types` | Optional single-choice stored as an array | Continue / deferred Step 5 | Recruiter matching context and CSV |
| Platforms / creator niches | All role paths | arrays plus `platform_other`, `niche_other` | Optional; Other text is conditional | Continue / deferred Step 5 | Matching context, searchable, CSV |
| Experience, start availability, portfolio | Seeker-only detailed path | scalar seeker context fields | Optional | Continue / deferred Step 5 | Seeker context and CSV |
| Hiring timeline, team size, website/channel | Recruiter-only detailed path | scalar recruiter context fields | Optional | Continue / deferred Step 5 | Recruiter context and CSV |
...
| Phone verification events | Legacy phone-verified rows only | verification status plus requested, last-sent and verified timestamps | New users receive format validation and save only; no phone OTP is entered | Legacy compatibility analytics only | Contact timeline in IST; no challenge secret is exposed/exported |
...
| First/last-touch attribution | Universal | versioned structured attribution JSONB; legacy fields retained | Bounded parameters; first touch retained, qualifying last touch updated | Step 1 / token-scoped resume sync | Separate touch detail, filters, source/campaign analytics and CSV |
...
- Legacy `whatsapp_consent` fields remain for compatibility. The current phone form
  records separate optional WhatsApp, SMS and voice choices in `phone_*_consent`,
  with version/time/source. These do not activate outbound sending.
...
Interests/context stage locally and write on Continue, with a best-effort unmount
save. This is not guaranteed per-selection persistence during abrupt termination.

The dashboard also supports explicitly confirmed deletion of one registration.
The server action rechecks administrator authorization; no bulk cleanup is implied.
