...
describe('legal pages', () => {
  it('renders the tailored privacy notice with the confirmed operator and contact', () => {
    ...
    expect(
      screen.getByRole('heading', { name: /Your brief is personal/i, level: 1 }),
    ).toBeInTheDocument();
    ...
    expect(screen.getAllByRole('link', { name: 'Cookie & storage notice' })[0]).toHaveAttribute(
      'href',
      '/cookies',
    );
  ...
  it('renders pre-launch terms without describing an operating marketplace', () => {
    ...
    expect(
      screen.getByRole('heading', { name: /waiting room/i, level: 1 }),
    ).toBeInTheDocument();
    ...
    expect(screen.getAllByRole('link', { name: 'Privacy Notice' })[0]).toHaveAttribute(
      'href',
      '/privacy',
    );
  });
  ...
  it('describes first-party storage and the no-banner conclusion', () => {
    ...
    expect(screen.getByRole('heading', { name: /browser keeps/i, level: 1 })).toBeInTheDocument();
